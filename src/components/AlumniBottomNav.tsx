...
function AlumniBottomNavInner({ isStaff }: AlumniBottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/98 backdrop-blur-xl border-t border-slate-200/90 shadow-[0_-6px_32px_rgba(0,61,122,0.10),0_-1px_0_rgba(0,0,0,0.04)] z-[1050]">
      <div className="flex justify-between items-center w-full max-w-2xl mx-auto px-1.5 pb-[env(safe-area-inset-bottom,10px)] pt-1.5">
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={href}
              className={`flex flex-col items-center justify-center py-1 px-0.5 flex-1 min-w-0 transition-all duration-200 relative group rounded-xl ${
                isActive ? '' : 'hover:bg-blue-50/70'
              }`}
            >
              {/* Active pill background */}
              {isActive && (
                <span className="absolute inset-0 rounded-xl bg-gradient-to-br from-[#003D7A]/10 to-[#C41E3A]/8 border border-[#003D7A]/10" />
              )}

              {/* Icon */}
              <span className={`relative z-10 flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-xl transition-all duration-200 ${
                isActive
              ...
              {/* Label */}
              <span className={`relative z-10 text-[9px] mt-0.5 tracking-tight font-bold truncate max-w-full text-center transition-colors duration-200 ${
                isActive
                  ? 'text-[#003D7A]'
              ...
              {isActive && (
                <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-gradient-to-r from-[#003D7A] to-[#C41E3A]" />
              )}
            </Link>
          );
        })}
